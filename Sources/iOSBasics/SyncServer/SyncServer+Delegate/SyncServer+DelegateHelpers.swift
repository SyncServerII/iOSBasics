...
extension SyncServer {
    func uploadCompletedHelper(_ delegated: AnyObject, file: Filenaming, result: Swift.Result<UploadFileResult, Error>) {
        ...
        case .success(let uploadFileResult):
            ...
            case .gone(_):
                do {
                    try cleanupAfterUploadCompleted(fileUUID: fileUUID, uploadObjectTrackerId: file.trackerId, result: .gone)
                } catch let error {
                    reportUploadError(fileUUID: fileUUID, trackerId: file.trackerId, error: error)
                    return
                }
            ...
            case .conflict(let conflictReason):
                guard let replacingFileUUID = try? UUID.from(conflictReason.replacingUUID),
                    let serverFileVersion = conflictReason.serverFileVersion else {
                    reportUploadError(fileUUID: fileUUID, trackerId: file.trackerId, error: SyncServerError.internalError("Did not have replacement fileUUID or fileVersion in Conflict"))
                    return
                }
                
                do {
                    try handleUploadConflict(originalFileUUID: fileUUID, replacingFileUUID: replacingFileUUID, serverFileVersion: serverFileVersion)
                } catch let error {
                    reportUploadError(fileUUID: fileUUID, trackerId: file.trackerId, error: error)
                    return
                }
                ...
                do {
                    try cleanupAfterUploadCompleted(fileUUID: replacingFileUUID, uploadObjectTrackerId: file.trackerId, result: .conflict)
                } catch let error {
                    reportUploadError(fileUUID: replacingFileUUID, trackerId: file.trackerId, error: error)
                    return
                }
                ...
                delegator { [weak self] delegate in
                    ...
                    delegate.uuidCollision(self, type: .file, from: fileUUID, to: replacingFileUUID)
                    
                    let result = UploadResult(fileUUID: replacingFileUUID, uploadType: .conflict)
                    delegate.uploadQueue(self, event: .completed(result))
                }
                
            case .success(let uploadResult):
                do {
                    try cleanupAfterUploadCompleted(fileUUID: fileUUID, uploadObjectTrackerId: file.trackerId,  result: .normal(uploadResult))
                } catch let error {
                    reportUploadError(fileUUID: fileUUID, trackerId: file.trackerId, error: error)
                    return
                }
...
extension SyncServer {
    ...
    enum UploadCleanup {
        case normal(UploadFileResult.Upload)
        case gone
        case conflict
    }
    ...
    func cleanupAfterUploadCompleted(fileUUID: UUID, uploadObjectTrackerId: Int64, result: UploadCleanup) throws {
        ...
        case .normal(let uploadResult):
            ...
            if v0Upload {
                try cleanupV0Uploads(objectTracker: objectTracker, uploadResult: uploadResult, sendPushNotifications: true)
            }
            else {
                // Need to wait on final cleanup until poll for deferred uploads indicates full completion.
                if uploadResult.uploadsFinished == .vNUploadsTransferPending {
                    logger.debug("result.deferredUploadId: \(String(describing: uploadResult.deferredUploadId))")
                    try objectTracker.update(setters: UploadObjectTracker.deferredUploadIdField.description <- uploadResult.deferredUploadId)
                }
                
                logger.debug("vN upload: result.uploadsFinished: \(uploadResult.uploadsFinished)")
            }

        case .conflict:
            guard objectTracker.v0Upload == true else {
                throw SyncServerError.internalError("Conflict currently should only occur with v0 uploads.")
            }
            
            // Don't send push notification in this case because someone else beat us to this upload.
            try cleanupV0Uploads(objectTracker: objectTracker, uploadResult: nil, sendPushNotifications: false)

        case .gone:
            // Just remove trackers when we get reports back from all of them.
            let fileTrackers = try objectTracker.dependentFileTrackers()
    ...
    private func cleanupV0Uploads(objectTracker: UploadObjectTracker, uploadResult: UploadFileResult.Upload?, sendPushNotifications: Bool) throws {
        ...
        if remainingUploads.count == 0 {
            ...
            if let uploadResult = uploadResult {
                if uploadResult.uploadsFinished != .v0UploadsFinished {
                    ...
                    logger.error("Did not get v0UploadsFinished when expected: \(uploadResult.uploadsFinished)")
                }
            }
            
            if sendPushNotifications, let pushNotificationMessage = objectTracker.pushNotificationMessage {
