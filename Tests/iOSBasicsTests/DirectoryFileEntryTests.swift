...
final class DirectoryFileEntryTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            ...
            entry = try DirectoryFileEntry(db: database, fileUUID: uuid, fileLabel: "One", fileGroupUUID: UUID(), fileVersion: 1, serverFileVersion: nil, deletedLocally: false, deletedOnServer: true, creationDate: Date(), goneReason: GoneReason.userRemoved.rawValue)
        } catch {
            XCTFail()
            return
        }
    ...
    func testFilterWhenTwoRowsFound() throws {
        ...
        let entry2 = try DirectoryFileEntry(db: database, fileUUID: UUID(), fileLabel: "Two", fileGroupUUID: UUID(), fileVersion: 1, serverFileVersion: nil, deletedLocally: false, deletedOnServer: true, creationDate: Date(), goneReason: GoneReason.userRemoved.rawValue)
